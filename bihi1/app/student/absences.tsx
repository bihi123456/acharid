...
export default function AbsencesScreen() {
  ...
  useEffect(() => {
    const loadAttendanceRecords = async () => {
      try {
        const data = await AsyncStorage.getItem('activeAttendance');
        if (data) {
          const attendance = JSON.parse(data);
          const studentRecords = attendance.attendanceRecords?.filter(
            (record: any) => record.studentId === studentUser?.id
          ) || [];

          const formattedRecords = studentRecords.map((record: any) => ({
            id: record.id,
